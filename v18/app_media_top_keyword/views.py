...
def home(request):
    return render(request, 'app_media_top_keyword/home.html')

# df_query should be global
@csrf_exempt
def api_get_media_top_keyword(request):
    ...
    df_query = filter_dataFrame_fullText(cate,media)
    print(len(df_query))
...
def filter_dataFrame_fullText(cate,media):

    # 新聞類別條件
    if (cate == "全部"):
        condition = (df['media'] == media) # "全部"類別不必過濾新聞種類
    else:
        # category新聞類別條件
        condition = (df['category'] == cate) & (df['media'] == media)
...
print("app_media_top_keyword was loaded!")
